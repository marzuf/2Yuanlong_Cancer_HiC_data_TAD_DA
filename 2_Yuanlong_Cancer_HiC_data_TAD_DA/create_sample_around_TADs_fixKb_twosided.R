...
script_name <- "create_sample_around_TADs_fixKb_twosided.R"
...
outFolder <- file.path("CREATE_SAMPLE_AROUND_TADS_FIXKB_TWOSIDED", bpAroundTAD)
...
if(buildSampleAroundTADs) {
  ...
  all_ds_sample_around_TADs_fixKb <-   foreach(ds = all_hicexpr_ds) %do% {
    ...
    all_sample_around_TADs <- foreach(reg = pipeline_tadList) %dopar% {
      ...
      all_dist_left <- abs(curr_genesOutsideDT_left$mid_pos-curr_midPos)
      stopifnot(!is.na(all_dist_left))
      
      all_dist_right <- abs(curr_genesOutsideDT_right$mid_pos-curr_midPos)
      stopifnot(!is.na(all_dist_right))
      
      sample_around_genes <- curr_genesOutsideDT$entrezID
      sample_around_genes_left <- curr_genesOutsideDT_left$entrezID
      sample_around_genes_right <- curr_genesOutsideDT_right$entrezID
      
      stopifnot(length(all_dist) == length(sample_around_genes_left) + length(sample_around_genes_right))
      
      stopifnot(setequal(c(sample_around_genes_left, sample_around_genes_right), sample_around_genes))
  ...
  sample_around_TADs_fixKb <- all_sample_around_TADs
  outFile <- file.path(outFolder, hicds, exprds, "sample_around_TADs_fixKb.Rdata")
  dir.create(dirname(outFile), recursive = TRUE)
  save(sample_around_TADs_fixKb, file=outFile)
  ...
  names(all_ds_sample_around_TADs_fixKb) <- all_hicexpr_ds
  outFile <- file.path(outFolder, "all_ds_sample_around_TADs_fixKb.Rdata")
  save(all_ds_sample_around_TADs_fixKb, file=outFile)
  cat(paste0("... written: ", outFile, "\n"))
  
  # end-if buildCorrAroundBD
} else {
  outFile <- file.path(outFolder, "all_ds_sample_around_TADs_fixKb.Rdata")
  all_ds_sample_around_TADs_fixKb <- eval(parse(text = load(outFile)))
}
