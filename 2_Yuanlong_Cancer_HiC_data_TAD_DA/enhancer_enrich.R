...
chromHMM_file <- file.path("enhancer_data", paste0("wgEncodeAwgSegmentationChromhmm", hmmdata, ".bed"))
...
if(buildData) {
  ...
  all_chromHMM_tadCount_DT <- foreach(hicds = all_hicds, .combine='rbind') %do% {
    ...
    ds_dt <- foreach(exprds = all_exprds[[paste0(hicds)]], .combine='rbind') %do% {
      ...
      combPval_file <- file.path(combPvalFolder, hicds, exprds, "meanCorr_meanLogFC_notAdjCombEmpPval.Rdata")
      stopifnot(file.exists(combPval_file))
      empPval_comb <- eval(parse(text = load(combPval_file)))
